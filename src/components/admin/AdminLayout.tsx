...
export function AdminLayout() {
  ...
  return <div className="min-h-screen bg-[#f8f8f7] text-foreground md:flex">
    ...
    <div className="min-w-0 flex-1"><header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border/80 bg-background/90 px-4 backdrop-blur md:px-8"><div className="flex items-center gap-3"><button onClick={() => setOpen(true)} className="grid h-9 w-9 place-items-center rounded-lg border border-border md:hidden"><Menu className="h-5 w-5" /></button><div><p className="text-[11px] font-bold text-muted-foreground">إدارة المتجر</p><h1 className="text-sm font-black">{title}</h1></div></div><div className="flex items-center gap-2"><Link to="/admin/categories" className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-border px-2.5 text-xs font-bold text-muted-foreground transition hover:border-primary/40 hover:text-primary"><FolderTree className="h-4 w-4"/><span className="hidden sm:inline">التصنيفات</span></Link><Link to="/admin/settings" className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-border px-2.5 text-xs font-bold text-muted-foreground transition hover:border-primary/40 hover:text-primary"><Settings className="h-4 w-4"/><span className="hidden sm:inline">الإعدادات</span></Link><Link to="/" className="hidden items-center gap-1 text-xs font-bold text-muted-foreground hover:text-primary sm:flex">عرض المتجر <ChevronLeft className="h-3.5 w-3.5"/></Link></div></header><main className="mx-auto max-w-7xl p-4 pb-24 md:p-8"><Outlet /></main><nav className="fixed inset-x-3 bottom-3 z-40 flex gap-1 overflow-x-auto rounded-2xl border border-border/80 bg-card/95 p-2 shadow-lift backdrop-blur md:hidden">{links.map(({ to, label, icon: Icon }) => <Link key={to} to={to} activeOptions={{ exact: to === "/admin" }} className="flex min-w-20 flex-1 flex-col items-center gap-1 rounded-xl px-2 py-2 text-[10px] font-bold text-muted-foreground" activeProps={{ className: "bg-primary/10 text-primary" }}><Icon className="h-4 w-4"/>{label}</Link>)}</nav></div>
  </div>;
}
